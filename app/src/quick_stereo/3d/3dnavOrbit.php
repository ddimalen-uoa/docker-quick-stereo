...
if($location=="./")
{
	$resultFile = "../".$_SESSION["cached_image"];
	$textureFile = "../".$_SESSION["cached_texture"];
	//echo "<p>$resultFile</p>";
}
else
{
...
function clock()
  {
	  ...
	  if(document.getElementById("fps").innerHTML == 'loading, if the 3D scene does not appear, press Refresh a few times'){	  
	  	document.location='3dnavOrbit.php?location=<? echo $location; ?>';
	  }
	  else{
...
<div id="bg">
  <input class="3DNavRefreshButton" type="button" name="button" id="button" onClick="document.location='3dnavOrbit.php?location=<? echo $location; ?>';" style="background:url(../img/Refresh.png); 
		width:50px; 
		height:50px;">
</div>
