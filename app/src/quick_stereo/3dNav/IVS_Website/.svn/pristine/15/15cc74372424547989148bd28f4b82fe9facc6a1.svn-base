<?php
//
// Character conversion functions of ConfTool
//
if (!defined('CONFTOOL')) die('Hacking attempt!');

/**
 * Convert from current character set to UTF-8, if it is not already UTF-8 and iconv is available!
 *
 * @param string $text string to be converted to UTF-8
 * @return string in UTF-8 format
 */
function ct_toUTF8 ( $text, $charset=null ) {
	global $ctconf;
	if (!$charset)
		$ct_charset = $ctconf['charset'];
	else
		$ct_charset = $charset;
	/* Fix Windows-1252 encoding: These characters fix the mapping of
	   the Windows-1252 characters in UTF-8 if ISO-8859-1 was assumed. */
	$cp1252_map = array(
	...
	if 	(function_exists('iconv') && strtoupper($ct_charset)!='UTF-8') {
		if (ct_strtoupper($ct_charset)=='ISO-8859-1')
			$ct_charset='cp1252'; // In fact most data is in Windows 1252 encoded!
		$text = iconv($ct_charset,"UTF-8",$text);
	} elseif (function_exists('mb_convert_encoding') && strtoupper($ct_charset)!='UTF-8') {
		if (ct_strtoupper($ct_charset)=='ISO-8859-1')
			$ct_charset='cp1252'; // In fact most data is in Windows 1252 encoded!
		$text = mb_convert_encoding($text,"UTF-8",$ct_charset);
	} elseif (strtoupper($ct_charset)=='ISO-8859-1') { // Use build in function of PHP
        $text = utf8_encode($text);
        // Convert CP1252 characters.
		if (ct_strtoupper($ct_charset)=='ISO-8859-1')
			$text = strtr($text, $cp1252_map);
	} // Failed! :-|
	return $text;
}

/**
 * Convert from current character set to ISO 8859-1, if it is not already ISO 8859-1 and iconv is available!
 *
 * @param string $text string to be converted to ISO 8859-1
 * @return string in ISO 8859-1 format
 */
function ct_toISO_8859_1 ( $text, $charset=null ) {
	global $ctconf;
	if (!$charset)
		$ct_charset = $ctconf['charset'];
	else
		$ct_charset = $charset;
	if 	(function_exists('iconv') && strtoupper($ct_charset)!='ISO-8859-1') {
		$text = iconv($ct_charset,"ISO-8859-1",$text);
	} elseif (function_exists('mb_convert_encoding') && strtoupper($ct_charset)!='ISO-8859-1') {
		$text = mb_convert_encoding($text,"ISO-8859-1",$ct_charset);
	} // no conversion...
	return $text;
}
...
/**
 * Convert hex and decimal encoded entities to UTF-8.
 * @param $text_to_convert Text to convert.
 * @return string converted
 */
function ct_decode_entities_to_UTF8($text) {
	$text = preg_replace('~&#x([0-9a-f]+);~ei', '_decode_entities_to_UTF8(hexdec("\\1"))', $text);
	$text = preg_replace('~&#([0-9]+);~e', '_decode_entities_to_UTF8(\\1)', $text);
	return $text;
}
// Helper function for the above.
function _decode_entities_to_UTF8($num) {
	if ($num > 0x7F && $num <= 0x7FF) 	return chr(($num>>0x06)+0xC0).chr(($num & 0x3F)+0x80);
	if ($num <= 0xFFFF) 	return chr(($num>>0x0C)+0xE0).chr((($num>>0x06)&0x3F)+0x80).chr(($num & 0x3F)+0x80);
	if ($num <= 0x1FFFFF) 	return chr(($num>>0x12)+0xF0).chr((($num>>0x0C)&0x3F)+0x80).chr((($num>>0x06)&0x3F)+0x80).chr(($num&0x3F)+0x80);
	return "&#$num;"; // Stings 0-127 shall not be decoded for security reasons!
}
...
//
//============================================================+
// Adapted for ConfTool by Harald Weinreich
 ...
 * @return string converted
 */
if (!function_exists("code_to_utf8")) {
 ...
 * @return string converted
 */
if (!function_exists("html_entity_decode_php4")) {
	function html_entity_decode_php4($text_to_convert, $quote_style=ENT_COMPAT) {
		...
		if ($quote_style==ENT_NOQUOTES) $return_text = str_replace('&quot;','"',$return_text);
		return $return_text;
	}
}
