...
window.onload = function() {
    ...
    imgObj.src = '../upload_stereo/left_raw_resized.jpg?' + (new Date()).getTime();
    ...
    imgDisp.src = '../upload_stereo/depthReserved.jpg?' + (new Date()).getTime();
...
function Colored() {
    ...
    for (var i = 0; i < disparityValues.length; i++) {
		...
		if( ((w+disparity*alpha) < WIDTH && (w+disparity*alpha) >= 0) && ((h+disparity*beta) < HEIGHT && (h+disparity*beta) >= 0) )
		{
			var newValue = value+4*(parseInt(h+disparity*beta)*WIDTH+parseInt(disparity*alpha));
		...
		/**/
		//var value = 4*(h*WIDTH+w);		
		/*if((h+disparity*beta) < HEIGHT && (h+disparity*beta) >= 0)
		{
			var newValue = 4*(parseInt(h+disparity*beta)*WIDTH+w);	
			data[newValue]   = originalData[value]; // red
			data[newValue+1] = originalData[value+1]; // green
			data[newValue+2] = originalData[value+2]; // blue			
		}*/
