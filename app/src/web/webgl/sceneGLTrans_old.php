 ...
 */
    ...
    function initBuffers() {
		...
		//var depthXX = 0;var depthYY = 0;var depthZZ = -100;
		
		var face = parseInt(facess[0]);
		var depthXX = -coordinatess[3*face];//cosPhi * sinTheta;
		var depthYY = coordinatess[3*face+1];//cosTheta;
		var depthZZ = -coordinatess[3*face+2];//(sinPhi * sinTheta); if(z < 0) z*= -1;
        ...
        for (var i=0; i < facess.length; i++) {    		
			...
			if(Math.abs(z-depthZZ) < 0.1)
			{
				...
				if(x < -0.4){
					...
					textureCoordData.push(u);
					textureCoordData.push(v);
					vertexPositionData.push(radius * depthXX);
					vertexPositionData.push(radius * (depthYY-offSetY));
					vertexPositionData.push(radius*depthZZ);
				}
    ...
    function drawScene(counter) {
        ...
        //if (lighting) 
		{
            gl.uniform3f(
                ...
                parseFloat(Math.abs(controlValue)),
                parseFloat(Math.abs(controlValue)),
                parseFloat(Math.abs(controlValue))
            );
    ...
    function tick() {
        ...
        drawScene(counter);
		counter++;
		//if(counter%100==0)		alert(counter);
    }
...
<canvas id="webgl_canvas" style="border: none;" width="800" height="600"></canvas>
</body>
