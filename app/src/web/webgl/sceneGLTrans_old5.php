...
if($location == ""){
	$location = '../quick_stereo/upload_stereo/'.$sessionLocation;
	$file = "../../quick_stereo/upload_stereo/".$sessionLocation."bigCoor.obj";
	$image->load("../../quick_stereo/upload_stereo/".$sessionLocation."left_raw_resized.jpg");
}
else{
...
<script type="text/javascript">
    ...
    function initGL(canvas) {
        ...
        if (!gl) 
		{
			...
			document.getElementById('showNoGL').style.display='';
			document.getElementById('webgl_canvas').style.display='none';
        }
		else{
			document.getElementById('webgl_canvas').style.display='';
		}
    }
 ...
 */
	...
	function handleDepth(delta) {
			if (delta > 0){
				if(depthRatio>0.2) depthRatio=depthRatio-0.1;
			}				
			else{
				depthRatio=depthRatio+0.1;
			}
	}
    ...
    function initBuffers() {		
        ...
        for (var i=0; i < facess.length; i=i+1) {  			
			...
			z = -depthRatio*coordinatess[3*face+2];			
			//z = -4.0*(1.0/((-coordinatess[3*face+2])+1.9))+2.2;//(sinPhi * sinTheta); if(z < 0) z*= -1;
    ...
    function drawScene(counter) {
        ...
        mat4.perspective(degree, 1, 0.1, 100.0, pMatrix);
        ...
        setMatrixUniforms();
		if(line)
			gl.drawElements(gl.LINES, objectItVertexIndexBuffer.numItems, gl.UNSIGNED_SHORT, 0);
		else
    ...
    function webGLStart() {
		document.getElementById('showNoGL').style.display='none';		
...
<?
$random=rand();
if(intval($_GET['delay'])>0){
	$wait = 2000+intval($_GET['delay'])*1000;
}
else{
	$wait = 2000;
}
?>
<body onLoad="setTimeout('webGLStart();',<? echo $wait; ?>);" bgcolor="#000000"  onMouseDown="changeLine(1)" onMouseUp="changeLine(0)">
<table width="800px" border="0" cellspacing="0" cellpadding="0" id="showNoGL">
  <?
  if($location == '../quick_stereo/upload_stereo/'){
  ?>
  <tr>    
    <td><!--<img src="<? echo "http://www.ivs.auckland.ac.nz/quick_stereo/upload_stereo/shareResults.jpg?".$random; ?>" width="800px">--></td>
  </tr>
  <?
  }
  else
  ...
  <!--
  <tr>
    <td><img src="<? echo "../".$location."_l_resized.jpg?".$random; ?>" width="400px"></td>
    <td><img src="<? echo "../".$location."_l_resized.jpg?".$random; ?>" width="400px"></td>
  </tr>
 
  <tr>
    <td><img src="<? echo "../".$location."_anaglyph.jpg?".$random; ?>" width="400px"></td>
    <td><img src="<? echo "../".$location."_p_stereogram.jpg?".$random; ?>" width="400px"></td>
  </tr>
   -->
   <tr>
    <td><img src="<? echo "../".$location."_anaglyph.jpg?".$random; ?>" width="100%"></td>    
  </tr>
  <? } ?>
</table>
<canvas id="webgl_canvas" style="border: none; display:none;" width="800" height="600"></canvas>
</body>
